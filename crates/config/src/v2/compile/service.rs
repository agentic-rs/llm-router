...
mod tests {
  ...
  #[test]
  fn request_limit_rejects_platform_usize_overflow() {
    assert!(matches!(
      compile_limit("service.request_limits.max_wire_bytes", u64::MAX),
      Err(CompileError::InvalidValue { location, .. })
        if location == "service.request_limits.max_wire_bytes"
    ));
  }
}
