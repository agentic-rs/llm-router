...
/// response body, which would terminate long-lived SSE streams. Callers must
/// use a separate bounded client for control-plane work and apply an execution
/// deadline to non-streaming requests.
...
/// This client is only correct when the response adapter preserves the
/// original headers and encoded bytes together.
